...
let horses = [
  ...
  { id: 'h2', name: 'Zafiro (Tranco Grande)', stride: 3.75, temp: 'Impulsivo' },
  { id: 'h3', name: 'Rayo (Tranco Corto)', stride: 3.45, temp: 'Calmo' }
];
...
function checkSafetyLimits(meters, strides) {
  if (strides === 1) {
    if (meters < 7.20) return "⚠️ RIESGO ALTO: < 7.20m (Peligro de choque en 1 tranco).";
    if (meters > 8.40) return "⚠️ DISTANCIA EXCESIVA: > 8.40m para 1 tranco.";
  } else if (strides === 2) {
    if (meters < 10.20) return "⚠️ RIESGO: < 10.20m (Demasiado apretada para 2 trancos).";
    if (meters > 11.80) return "⚠️ DISTANCIA EXCESIVA: > 11.80m para 2 trancos.";
  }
  return null;
}
