...
let horses = [
  { id: 'h1', name: 'Caballo Estándar', stride: 3.60, temp: 'Normal' },
  { id: 'h2', name: 'Zafiro (Batida Grande)', stride: 3.75, temp: 'Impulsivo' },
  { id: 'h3', name: 'Rayo (Batida Corta)', stride: 3.45, temp: 'Calmo' }
];
...
function renderHorseSelect() {
  ...
  horses.forEach(h => {
    ...
    opt.value = h.id;
    opt.innerText = `${h.name} (${h.stride} m - ${h.temp || 'Normal'})`;
    select.appendChild(opt);
  });
  
  if (horses.length > 0 && document.getElementById('horse-stride')) {
...
function setupEventListeners() {
  ...
  if (horseSelect) {
    horseSelect.addEventListener('change', (e) => {
      ...
      if (selected && document.getElementById('horse-stride')) {
        document.getElementById('horse-stride').value = selected.stride;
      }
    });
  }

  if (document.getElementById('human-step')) document.getElementById('human-step').addEventListener('input', syncDistances);
  if (document.getElementById('distance-meters')) document.getElementById('distance-meters').addEventListener('input', () => syncFrom('meters'));
  if (document.getElementById('distance-steps')) document.getElementById('distance-steps').addEventListener('input', () => syncFrom('steps'));

  if (document.getElementById('role-rider')) document.getElementById('role-rider').addEventListener('click', () => setRole('rider'));
  if (document.getElementById('role-designer')) document.getElementById('role-designer').addEventListener('click', () => setRole('designer'));

  if (document.getElementById('mode-meters')) document.getElementById('mode-meters').addEventListener('click', () => setInputMode('meters'));
  if (document.getElementById('mode-steps')) document.getElementById('mode-steps').addEventListener('click', () => setInputMode('steps'));

  if (document.getElementById('btn-manage-horses')) {
  ...
  if (document.getElementById('btn-close-horse-manager')) {
  ...
  if (document.getElementById('btn-save-horse')) document.getElementById('btn-save-horse').addEventListener('click', addNewHorse);
  if (document.getElementById('btn-delete-horse')) document.getElementById('btn-delete-horse').addEventListener('click', deleteSelectedHorse);

  if (document.getElementById('btn-start-course')) document.getElementById('btn-start-course').addEventListener('click', startCourse);
  if (document.getElementById('btn-edit-setup')) document.getElementById('btn-edit-setup').addEventListener('click', editSetup);

  if (document.getElementById('btn-calculate')) document.getElementById('btn-calculate').addEventListener('click', calculateLine);
  if (document.getElementById('btn-save-to-course')) document.getElementById('btn-save-to-course').addEventListener('click', saveLineToLog);
  if (document.getElementById('btn-reset-course-log')) document.getElementById('btn-reset-course-log').addEventListener('click', resetCourseLog);
}
...
function setRole(role) {
  ...
  if (role === 'rider') {
    ...
    if (btnDesigner) btnDesigner.classList.remove('active');
    if (formRider) formRider.classList.remove('hidden');
    if (formDesigner) formDesigner.classList.add('hidden');
  } else {
    if (btnDesigner) btnDesigner.classList.add('active');
    if (btnRider) btnRider.classList.remove('active');
    if (formDesigner) formDesigner.classList.remove('hidden');
    if (formRider) formRider.classList.add('hidden');
  }
}

function syncFrom(source) {
  const humanStepEl = document.getElementById('human-step');
  const humanStep = humanStepEl ? (parseFloat(humanStepEl.value) || 1.0) : 1.0;
  const metersInput = document.getElementById('distance-meters');
  const stepsInput = document.getElementById('distance-steps');

  if (!metersInput || !stepsInput) return;
...
function setInputMode(mode) {
  ...
  if (mode === 'meters') {
    if (btnMeters) btnMeters.classList.add('active');
    if (btnSteps) btnSteps.classList.remove('active');
    if (groupMeters) groupMeters.classList.remove('hidden');
    if (groupSteps) groupSteps.classList.add('hidden');
    syncFrom('steps');
  } else {
    if (btnSteps) btnSteps.classList.add('active');
    if (btnMeters) btnMeters.classList.remove('active');
    if (groupSteps) groupSteps.classList.remove('hidden');
    if (groupMeters) groupMeters.classList.add('hidden');
    syncFrom('meters');
  }
}

function addNewHorse() {
  const nameEl = document.getElementById('new-horse-name');
  const strideEl = document.getElementById('new-horse-stride');
  if (!nameEl || !strideEl) return;

  const name = nameEl.value.trim();
  const stride = parseFloat(strideEl.value);
  ...
  const newHorse = { id: 'h_' + Date.now(), name, stride, temp: 'Normal' };
...
function calculateLine() {
  const selectedHorseId = document.getElementById('horse-select')?.value;
  const currentHorse = horses.find(h => h.id === selectedHorseId) || horses[0];
  ...
  let tempDelta = 0.00;
  if (currentHorse && currentHorse.temp === 'Impulsivo') tempDelta = 0.05;
  if (currentHorse && currentHorse.temp === 'Calmo') tempDelta = -0.05;

  let effectiveStride = horseStride + tempDelta;
...
function checkSafetyLimits(meters, strides) {
  if (strides === 1) {
    if (meters < 7.20) return "⚠️ RIESGO ALTO: < 7.20m (Peligro de choque en 1 batida).";
    if (meters > 8.40) return "⚠️ DISTANCIA EXCESIVA: > 8.40m para 1 batida.";
  } else if (strides === 2) {
    if (meters < 10.20) return "⚠️ RIESGO: < 10.20m (Demasiado apretada para 2 batidas).";
    if (meters > 11.80) return "⚠️ DISTANCIA EXCESIVA: > 11.80m para 2 batidas.";
  }
  return null;
}
...
function calculateAsRider(effStride, humanStep, R, B, jump1, jump2, speed, lineLabel, elemType) {
  ...
  let adviceText = generateAdvice(stridesText(numberOfStrides, elemType), diff, speed, trajectory, jump1, jump2);
  const safetyWarn = checkSafetyLimits(realDistanceMeters, numberOfStrides);
  if (safetyWarn) adviceText = `${safetyWarn}\n\n${adviceText}`;
...
function stridesText(strides, elemType) {
  let label = `${strides} ${strides === 1 ? 'BATIDA' : 'BATIDAS'}`;
...
function renderRiderResults(realDist, usefulDist, strides, diff, steps, trajectory, jump1, jump2, speed, elemType, advice) {
  ...
  document.getElementById('recommendation-text').innerText = advice.replaceAll('**', '');
...
function calculateAsDesigner(effStride, humanStep, R, B, jump1, jump2, speed, lineLabel, elemType) {
  ...
  lastCalculatedLine = {
    ...
    diff: 0,
    role: 'designer',
    advice: `Diseño para ${targetStrides} batidas a ${speed} m/min.\n💡 ${adviceNote}`
  };
...
function renderDesignerResults(meters, steps, strides, feel, R, B, effStride, adviceNote, speed) {
  ...
  document.getElementById('badge-strides').innerText = `DISEÑO: ${strides} BATIDAS`;
  ...
  document.getElementById('label-metric-4').innerText = `Batida (${speed} m/min)`;
  ...
  let finalAdvice = `Para armar esta línea de ${strides} batidas a ${speed} m/min, debes colocar los saltos a ${meters.toFixed(2)} metros (o ${steps.toFixed(1)} pasos caminados).\n\n💡 Intención Técnica: ${adviceNote}`;

  document.getElementById('badge-status').innerText = badgeLabel;
  document.getElementById('badge-status').className = `badge ${statusClass}`;
  document.getElementById('recommendation-text').innerText = finalAdvice.replaceAll('**', '');
  ...
  courseLog.forEach((item, index) => {
    ...
    div.innerHTML = `
      <div class="log-header">
        <div>
          <div class="log-title">#${index + 1}. ${item.label} (${item.strides} batida/s)</div>
        ...
        <span class="badge ${item.role === 'rider' ? (Math.abs(item.diff) <= 0.22 ? 'badge-normal' : (item.diff < 0 ? 'badge-short' : 'badge-long')) : 'badge-normal'}">${item.strides} B</span>
      </div>
      <div class="log-detail hidden" id="log-detail-${index}">
        <strong>💡 Recomendación Táctica:</strong><br>${item.advice.replaceAll('**', '')}
      </div>
    `;
