let currentRole = 'rider'; // 'rider' | 'designer'
...
function setupEventListeners() {
  ...
  // ALTERNAR ROL (Jinete vs Armador)
...
// MODO JINETE: De distancia a tramos
function calculateAsRider(effStride, humanStep, R, B, jump1, jump2, speed, lineLabel, elemType) {
  ...
  renderRiderResults(realDistanceMeters, usefulDistance, numberOfStrides, diff, stepsCount, trajectory, jump1, jump2, speed, elemType);
}

function renderRiderResults(realDist, usefulDist, strides, diff, steps, trajectory, jump1, jump2, speed, elemType) {
  ...
  if (absDiff <= 0.22) {
    ...
    advice = `Línea de ${strides} tramos fluida a ${speed} m/min. Mantén el ritmo constante.`;
  } else if (diff < -0.22) {
...
// MODO ARMADOR: De tramos deseados a metros exactos
