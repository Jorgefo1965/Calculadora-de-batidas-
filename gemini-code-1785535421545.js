...
function calculateLine() {
  ...
  // CORRECCIÓN PRECISA: SI EL PASO ES 1.00m, COINCIDEN EXACTAMENTE PASOS Y METROS
  if (currentInputMode === 'meters') {
    realDistanceMeters = parseFloat(document.getElementById('distance-meters').value);
    if (humanStep === 1.00) {
      stepsCount = realDistanceMeters;
    } else {
      stepsCount = Math.round(realDistanceMeters / humanStep);
    }
  } else {
    stepsCount = parseFloat(document.getElementById('distance-steps').value);
    realDistanceMeters = stepsCount * humanStep;
  }
  ...
  // Forzar 1 tramo si es doble combinación cerrada
...
// GUARDAR EN LA BITÁCORA DEL RECORRIDO
  ...
  courseLog.forEach((item, index) => {
    ...
    div.innerHTML = `
      <div>
        ...
        <div class="log-sub">${item.meters.toFixed(2)}m • ${item.steps} pasos • ${statusNote} (${diffSign}${item.diff.toFixed(2)}m)</div>
      </div>
