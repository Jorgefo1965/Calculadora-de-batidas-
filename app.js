...
function calculateLine() {
  ...
  // LÓGICA DE CONVERSIÓN DIRECTA SIN DESFASES DE REDONDEO
  if (currentInputMode === 'meters') {
    realDistanceMeters = parseFloat(document.getElementById('distance-meters').value);
    stepsCount = (humanStep > 0) ? (realDistanceMeters / humanStep) : realDistanceMeters;
  } else {
...
function renderLineResults(realDist, usefulDist, strides, diff, steps, trajectory, jump1, jump2, speed, elemType) {
  ...
  // Formato limpio: si el valor de pasos es entero o coincide a 1.00m, no muestra decimales feos
